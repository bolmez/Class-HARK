...
#os.chdir('/Users/Pavel/Documents/Class-HARK/')
#os.path.abspath("/Users/Pavel/Documents/Class-HARK/")
...
class cstwKinkedRagent(Model.KinkedRconsumerType):
    ...
    def simulateCSTW(self):
        ...
        none
        '''
#        #!!! Call it to provide agent with Rfree
#        
#        # Unpack the income distribution and get average and worst outcomes # Line 2017 IndShockModel
#        PermShkValsNext   = self.IncomeDstn[0][1]
#        TranShkValsNext   = self.IncomeDstn[0][2]
#        PermShkMinNext    = np.min(PermShkValsNext)    
#        TranShkMinNext    = np.min(TranShkValsNext)        
#        
#        # Calculate the infinite horizon natural borrowing constraint # Line 2027 IndShockModel
#        temp              = self.PermGroFac[0]*PermShkMinNext/self.Rboro
#        self.BoroCnstNat       = -TranShkMinNext*temp/(1.0-temp)        
#        
#        # Make a grid of end-of-period assets, including *two* copies of a=0 # Line 1325 IndShockModel
#        self.mNrmMinNow = np.max([self.BoroCnstNat]) # Line 729 IndShockModel; uses max(BoroCnstNat,BoroCnstArt)  
#        aNrmNow           = np.sort(np.hstack((np.asarray(self.aXtraGrid) + 
#                            self.mNrmMinNow,np.array([0.0,0.0]))))
#        aXtraCount        = aNrmNow.size        
#        
#        # Make a 1D array of the interest factor at each asset gridpoint    # Line 1332 IndShockModel
...
#        self.Rfree        = Rfree_vec        
        ...
        self.initializeSim()
        self.simConsHistory()
        #self.W_history = self.pHist*self.bHist/self.Rfree #!!! Need to be modified for Rboro and Rsave
        self.W_history = self.pHist*self.bHist
        self.W_history[self.W_history <= 0] /= self.Rboro # Notice the divide AND assign operator /=
        self.W_history[self.W_history > 0] /= self.Rsave
#        print('Max of self.W_history is ' + str(self.W_history.max()))
#        print('Min of self.W_history is ' + str(self.W_history.min()))
...
def calculatePctHHassetsDifference(sim_wealth,weights,target_pctHHassets):
    ...
    # Proportion of households having ALMOST zero liquid assets (assuming that
    # it is unlikely that the simulation yields HH with exactly zero wealth)
    #sim_pctHHassets = float(np.sum(-1 <= i <= 1 for i in sim_wealth))/len(sim_wealth)
    sim_pctHHassets = float((sim_wealth == 0).sum())/sim_wealth.size
    #sim_pctHHassets = float(np.sum(i == 0 for i in sim_wealth))/len(sim_wealth)  # First attempt          
    #self.W_history[self.W_history <= 0] /= self.Rboro # Just for reference
    # Check limits of simulated wealth
    distance = (sim_pctHHassets - target_pctHHassets)**1.0
#    print(sim_wealth.size)
#    print('sum of equal zero is :' + str(float((sim_wealth <= 0).sum())))
#    print('length of sim_wealth is :' + str(len(sim_wealth)))
#    print('sim_pctHHassets is :' + str(sim_pctHHassets))
#    print('Max of sim_wealth is ' + str(sim_wealth.max()))
#    print('Min of sim_wealth is ' + str(sim_wealth.min()))    
#    print('PctHHassetsDifference is: ' + str(distance))
    return distance
...
# Define the main simulation process for matching the proportion of HH having 
# zero liquid assets
...
def makeCSTWresultsRboro(Rboro,nabla,save_name=None):
    '''
    Produces a variety of results for the cstwKinkedR model (usually after estimating).
    ...
    none
    '''
    #!!! Once Rboro and nabla are obtained through estimation, generate a 
    # distribution of Rboro that will generate a good approx to the empirical distribution
...
def makeCSTWstats(Rboro,nabla,this_type_list,age_weight,lorenz_distance=0.0,save_name=None):
    '''
    ...
    nabla : float
        Width of the uniform distribution of rates on borrowing
    this_type_list : [cstwKinkedRagent]
    ...
    if Params.do_Rboro_dist and Params.do_lifecycle:
...
#    ----------
#    parameter : string
#        Name of an attribute/parameter of cstwKinkedRagent on which to perform a
...
#        is an element of cstwKinkedRagent.time_vary).  While the sensitivity analysis
...
#        DropoutType = cstwKinkedRagent(**Params.init_dropout)
...
#        else:
#            top = 0.998
#        DiscFac_new = brentq(intermediateObjective,0.90,top,xtol=10**(-8)) #!!! Finds the nabla that minimizes the distance with the target
#        N=Params.pref_type_count
...
os.chdir('/Users/Pavel/Documents/Class-HARK/cstwMPC/')
import SetupParamsCSTW as Params
Params.do_lifecycle = False            # Perpetual youth 
Params.do_tractable = False 

Params.do_Rboro_dist = False                    # Do Rboro-dist version if True, Rboro-point if False
...
Params.percentiles_to_match = [0.2, 0.4, 0.6, 0.8] # Which points of the Lorenz curve to match in Rboro-dist (must be in (0,1))
...
if Params.run_estimation:
    ...
    #spec_name=None
    makeCSTWresultsRboro(Rboro,nabla,save_name=spec_name) #None) #spec_name) in order to save figures
